...
@click.command(
    help="Create CSV file for 'Variational approximation in H3 compared to MCMC'"
)
@click.option("--output", type=click.File("w"), help="CSV output file")
@click.argument("tree_files", nargs=-1)
def main(output, tree_files):
    treelists_names = []
    for f in tree_files:
        m = re.search(r"run(\d).t", f)
        if m is None:
            treelists_names.append("Dodonaphy")
        else:
            treelists_names.append(f"MB{m.group(1)}")
    ...
    for i, treelist in enumerate(treelists):
        (
            mean_edge_lengths_treelist,
            std_edge_lengths_treelist,
            count_edge_lengths_treelist,
        ) = calculate_mean_edge_lengths(treelist, all_bit_strings)
    ...
    for bit_string in all_bit_strings:
        ...
        for i in range(len(treelists)):
            mean_length = mean_edge_lengths_by_treelist[treelists_names[i]].get(
                bit_string, np.nan
            )
            row.append(mean_length)
            std_length = std_edge_lengths_by_treelist[treelists_names[i]].get(
                bit_string, np.nan
            )
            row.append(std_length)
            count_length = count_edge_lengths_by_treelist[treelists_names[i]].get(
                bit_string, np.nan
            )
    ...
    sorted_df = pd.concat([df.dropna(), df[df.isna().any(axis=1)]])
